...
`Tab/Shift+Tab` 只切换主导航与明细导航等标签层级，左右键直接切换本层标签页，向下键
进入内容区，上下键浏览内容，`Enter/Space` 选择或激活内容项，`PgUp/PgDn` 对当前内容组翻页；
...
字符并在天之间留一列空隙；零用量使用弱化色，所有非零档位统一使用绿色。概览始终只
显示一行按日期排列的数据，每天固定占 3 列，按可用宽度显示最新能容纳的天数；完整 371 天
历史仍在“明细 → 活动”中分页查看。relay 与个人钱包设置继续通过 `/settings` 和 domain 公共兼容
